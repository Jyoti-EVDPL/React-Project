...
function ConfigurationEdit(props) {
    ...
    const config_id = location.state.config_id;
    console.log("hyy")
    console.log(config_id);
    console.log(location.state)

    const saveNotes = async (e) => {
        // const { config_id, configname, title} = state;
        // console.log(config_id)
        ...
        if ((name === "" || title === "" || description === "")) {
            ...
            e.preventDefault();
            console.log("config_id,name,title, description", config_id, name, title, description)
            let result = await fetch('http://localhost:4000/auth/home/config/update', {
                method: 'post',
                body: JSON.stringify({ config_id, name, title, description }),
                headers: {
